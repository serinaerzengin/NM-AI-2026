...
wrapper.context for your own mutable app state and dependencies.
wrapper.usage for aggregated request and token usage across the current run.
wrapper.tool_input for structured input when the current run is executing inside Agent.as_tool().
wrapper.approve_tool(...) / wrapper.reject_tool(...) when you need to update approval state programmatically.
