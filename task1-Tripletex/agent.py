...
Return a JSON object with the corrected "payload" (or "params" for query-param endpoints).
If the step should be skipped, return {"skip": true}.
...
async def plan(llm_client, prompt: str, file_parts: list[dict], registry: dict) -> dict:
    ...
    index = load_index()
    domain = load_domain()
    system = PLAN_SYSTEM.replace("{domain}", domain).replace("{index}", index)
...
def _sanitize_step(path: str, method: str, payload: dict | None, params: dict | None) -> tuple[dict | None, dict | None]:
    ...
    if "/:payment" in path:
        if payload and isinstance(payload, dict) and not params:
            params = {}
    ...
    elif "/voucher" in path and payload and isinstance(payload, dict):
    ...
    # ── accountingDimensionName: remap "name" → "dimensionName" ──
    elif "/accountingDimensionName" in path and payload and isinstance(payload, dict):
        if "name" in payload and "dimensionName" not in payload:
            payload["dimensionName"] = payload.pop("name")

    # ── accountingDimensionValue: remap "name" → "displayName" ──
    elif "/accountingDimensionValue" in path and payload and isinstance(payload, dict):
    ...
    # ── GET /ledger/account: ensure narrow range to avoid huge result sets ──
        ...
        if params and ("numberFrom" not in params and "numberTo" not in params):
            # LLM forgot to narrow — default to a reasonable range if we see no filter
...
async def execute(llm_client, tripletex: TripletexClient, steps: list[dict], registry: dict):
    ...
    for i, step in enumerate(steps):
        ...
        if payload is not None and method in ("POST", "PUT"):
            ...
            if result["errors"]:
                ...
                payload = await fix_payload(llm_client, step, result["errors"], tripletex.state, payload, registry)
                if payload is None:
        ...
        if api_result["status"] < 400:
            ...
            for retry in range(MAX_RETRIES_PER_STEP):
                fixed = await fix_payload(
                    llm_client, step, [str(api_result["data"])], tripletex.state, last_payload, registry
                )
                if fixed is None:
                    break
...
async def fix_payload(llm_client, step: dict, errors: list[str], state: dict,
                      resolved_payload: dict | None = None, registry: dict | None = None) -> dict | None:
    ...
    system = EXECUTE_FIX_SYSTEM.replace("{state}", state_summary or "(empty)").replace("{schema_section}", schema_section)
...
async def run(prompt: str, base_url: str, session_token: str, files: list[dict] | None = None):
    ...
    try:
        ...
        await execute(llm_client, tripletex, steps, registry)
